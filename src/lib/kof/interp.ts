...
export class KofInterpreter {
  ...
  private defFn(d: Fn): void {
    ...
    if (clash)
      throw new KofDiag(`função \`${d.name}\` declarada duas vezes com a mesma assinatura`, "SEM047", d.line);
    list.push(d);
  ...
  private defClass(d: ClassDecl): void {
    ...
    this.classes.set(d.name, { decl: d, fields, ctor, methods, prim: d.primary, superName: d.superName });
  }
  ...
  private async exec(n: Node, scope: Scope, self: KofObj | null): Promise<void> {
    ...
    switch (n.kind) {
      ...
      case "ForIn": {
        ...
        if (typeof it === "string")
          throw new KofDiag("for-in sobre String é rejeitado — String não é coleção", "SEM058", n.line);
  ...
  private async assignTarget(target: Expr, v: KofVal, scope: Scope, self: KofObj | null): Promise<void> {
    if (target.kind === "Id") {
      if (!scope.has(target.name))
        throw new KofDiag(`variável \`${target.name}\` não declarada — use \`var\``, "SEM011", target.line);
    ...
    if (target.kind === "Field") {
      ...
      if (isObj(obj)) {
        if (obj.kind === "record")
          throw new KofDiag(`record \`${obj.recType}\` é imutável — campo \`${target.name}\``, "SEM033", target.line);
      ...
      if (isList(obj)) {
        if (idx < 0 || idx >= obj.length) {
          ...
          throw new KofRuntimeError(
            isArr ? `Array index out of bounds: ${idx} (length ${obj.length})` : `Index out of bounds: ${idx} (size ${obj.length})`,
          );
        }
        obj[idx] = v;
        return;
    ...
    throw new KofDiag("alvo de atribuição inválido", "PARSE010", (target as { line?: number }).line ?? 0);
  }

  private async assign(target: Expr, op: string, v: KofVal, scope: Scope, self: KofObj | null): Promise<void> {
  ...
  private async matchPattern(pat: Pattern, subject: KofVal, scope: Scope, self: KofObj | null): Promise<Scope | null> {
    const inner = new Scope(scope);
    ...
    if (pat.type === "record" && pat.recType) {
      if (!isObj(subject) || subject.kind !== "record" || subject.recType !== pat.recType) return null;
  ...
  async eval(e: Expr, scope: Scope, self: KofObj | null): Promise<KofVal> {
    ...
    switch (e.kind) {
      ...
      case "Binary":
        return this.binary(e.op, await this.eval(e.left, scope, self), await this.eval(e.right, scope, self), e.line);
      case "Assign": {
      ...
      case "IfExpr": {
        if (truthy(await this.eval(e.cond, scope, self))) return await this.eval(e.then, scope, self);
      ...
      case "NewArray": {
        ...
        const zero: KofVal =
          e.elem === "Long" ? 0n : e.elem === "Bool" ? false : (e.elem === "Double" || e.elem === "Float" ? 0.0 : e.elem === "Char" ? ({ __kofChar: 0 } as KofChar) : e.elem === "Int" || e.elem === "Short" || e.elem === "Byte" ? 0 : null);
      ...
      case "Index": {
        ...
        if (isMap(obj) || isSet(obj))
          throw new KofDiag(`subscript em ${isMap(obj) ? "Map" : "Set"} é rejeitado — use .get(k)/iterar`, "SEM054", e.line);
        if (isList(obj)) {
          const isArr = (obj as KofList & { __kofArray?: boolean }).__kofArray;
          if (idx < 0 || idx >= obj.length)
            throw new KofRuntimeError(
      ...
      default:
        throw new KofDiag(`expressão não suportada no playground (${(e as { kind: string }).kind})`, "PARSE041", (e as { line?: number }).line ?? 0);
    }
  }

  private evalId(name: string, line: number, scope: Scope, self: KofObj | null): KofVal {
    if (scope.has(name)) return scope.get(name) ?? null;
    if (STDLIB_NAMESPACES.includes(name) || name === "json" || name === "db" || name === "http" || name === "web" || name === "String" || name === "Object" || name === "Int" || name === "Long" || name === "Double" || name === "Float" || name === "Boolean")
  ...
  private async evalField(objE: Expr, name: string, line: number, scope: Scope, self: KofObj | null): Promise<KofVal> {
    ...
    if (objE.kind === "Id" && (STDLIB_NAMESPACES.includes(objE.name) || ["json", "String", "Int", "Long", "Double", "Float", "Boolean", "Object"].includes(objE.name))) {
  ...
  private getField(obj: KofVal, name: string, line: number, scope: Scope, self: KofObj | null): KofVal {
    if (obj === null || obj === undefined) throw new KofDiag("dereference de null", "SEM049", line);
    if (isObj(obj) && typeof obj.ns === "string" && (STDLIB_NAMESPACES.includes(obj.ns) || obj.ns === "json" || ["String", "Int", "Long", "Double", "Float", "Boolean", "Object"].includes(obj.ns))) {
    ...
    if (isList(obj)) {
      ...
      if (k.__kofArray && ["add", "get", "set", "remove", "clear", "map", "filter", "reduce"].includes(name))
        throw new KofDiag(`array é fixo — sem \`.${name}\`: use arr[i] e arr.length`, "SEM028", line);
      if (["add", "push", "append", "get", "set", "contains", "remove", "clear", "indexOf", "map", "filter", "reduce", "forEach", "toString", "size"].includes(name))
        return kofObj("closure", { listOp: name, recv: k, scope, self });
      throw new KofDiag(`List não tem \`.${name}\``, "SEM011", line);
    }
    if (isMap(obj)) {
      if (["put", "get", "remove", "contains", "containsKey", "size", "keys", "values", "clear", "isEmpty", "forEach", "toString"].includes(name))
    ...
    if (isSet(obj)) {
      if (["add", "contains", "remove", "size", "clear", "isEmpty", "forEach", "toString"].includes(name))
    ...
    if (isObj(obj)) {
      ...
      if (obj.kind === "record") {
        ...
        if (comp.some((c) => c.name === name)) return kofObj("closure", { recAccessor: obj, field: name });
        if (name === "toString" || name === "equals" || name === "hashCode") return kofObj("closure", { recSpecial: name, recv: obj });
        throw new KofDiag(`record \`${obj.recType}\` não tem componente \`${name}\``, "SEM011", line);
      }
      if (name in obj && name !== "toString") return (obj as Record<string, unknown>)[name] as KofVal;
      if (obj.kind === "object" && typeof obj.clsType === "string" && this.classes.has(String(obj.clsType))) {
    ...
    if (typeof obj === "string") {
      ...
      if (["charAt", "substring", "contains", "startsWith", "endsWith", "indexOf", "lastIndexOf", "toUpperCase", "toLowerCase", "trim", "replace", "split", "equals", "equalsIgnoreCase", "compareTo", "isEmpty", "hashCode", "toString", "toInt", "toLong", "toDouble", "toFloat"].includes(name))
        return kofObj("closure", { strOp: name, recv: obj, scope, self });
      throw new KofDiag(`String não tem \`.${name}\``, "SEM011", line);
    }
    if (typeof obj === "number" && !Number.isInteger(obj) || isFp(obj as Val | FpVal) || typeof obj === "bigint" || typeof obj === "boolean" || isKofChar(obj)) {
      if (name === "toString") return kofObj("closure", { toStringOf: obj });
  ...
  private async evalCall(callee: Expr, argExprs: Expr[], scope: Scope, self: KofObj | null, line: number): Promise<KofVal> {
    const args: KofVal[] = [];
    ...
    if (callee.kind === "Method") {
      ...
      if (isObj(recv) && typeof recv.ns === "string" && this.isStaticNs(recv.ns)) {
        return this.staticCall(recv.ns, recv.member ? String(recv.member) : callee.name, args, line);
      }
  ...
  private async staticCall(ns: string, fn: string, args: KofVal[], line: number): Promise<KofVal> {
    ...
    if (ns === "db")
      throw new KofRuntimeError("kof.db exige libsqlite3/JDBC do host — no browser/JS puro é o gap DB001 (o `kof check` oficial reporta em compile-time)", undefined, true);
    if (ns === "http") return this.httpCall(fn, args);
    if (ns === "web")
      throw new KofRuntimeError("kof.web server roda no `kof serve` (JVM) — WEB001/WEB004 nos outros targets; no browser é gap honesto", undefined, true);
    if (ns === "String") {
      if (fn === "valueOf") return kofStr(args[0] ?? null);
      if (fn === "format") throw new KofRuntimeError("String.format: gap COMP002 no target JS puro (paridade §239)", undefined, true);
    ...
    if (["Int", "Long", "Double", "Float", "Boolean"].includes(ns)) {
      ...
      if (fn === "parseInt" || fn === "parseLong" || fn === "parseDouble" || fn === "parseFloat" || fn === "parseBoolean") {
        return parseStatic(ns, fn, s);
      }
      if (fn === "toString" && (ns === "Int" || ns === "Long")) {
        const radix = args[1] !== undefined ? Number(asInt(args[1])) : 10;
        return (typeof args[0] === "bigint" ? args[0] : BigInt(wrapInt(asInt(args[0] ?? 0)))).toString(radix >>> 0 ? radix : 10);
      }
      if (fn === "toHexString") return (typeof args[0] === "bigint" ? args[0] : BigInt(wrapInt(asInt(args[0] ?? 0)) >>> 0)).toString(16);
      if (fn === "toBinaryString") return (typeof args[0] === "bigint" ? args[0] : BigInt(wrapInt(asInt(args[0] ?? 0)) >>> 0)).toString(2);
  ...
  private async httpCall(fn: string, args: KofVal[]): Promise<KofVal> {
    ...
    if (typeof fetch === "undefined")
      throw new KofRuntimeError(`HTTP003: kof.http.${fn} — sem transporte neste ambiente (o browser tem fetch; Node do playground não rodou aqui)`, undefined, true);
    const method = { get: "GET", post: "POST", put: "PUT", delete: "DELETE", patch: "PATCH", options: "OPTIONS" }[fn] ?? "GET";
    const body = fn === "post" || fn === "put" || fn === "patch" ? (args[1] !== undefined ? kofStr(args[1]) : undefined) : undefined;
  ...
  private jsonCall(fn: string, args: KofVal[], line: number): KofVal {
    ...
    if (fn === "decode") {
      ...
      try {
        ...
        throw new KofRuntimeError(`json.decode: JSON inválido: ${text.slice(0, 40)}`, undefined, true);
      }
  ...
  private async callByName(name: string, args: KofVal[], scope: Scope, self: KofObj | null, line: number): Promise<KofVal | typeof NO_MATCH> {
    const NO = NO_MATCH;
    if (name === "println") {
  ...
  private invokeFunc(overloads: Fn[], args: KofVal[], line: number): Promise<KofVal> | KofVal {
    ...
    if (candidates.length === 0) {
      if (overloads.length > 1) throw new KofDiag("chamada ambígua entre overloads", "SEM057", line);
      const o = overloads[0]!;
      const required = o.params.filter((p) => !p.def).length;
      if (args.length < required || args.length > o.params.length)
        throw new KofDiag(`\`${o.name}\` espera ${required}-${o.params.length} argumento(s), recebeu ${args.length}`, "SEM013", line);
  ...
  private async invoke(f: KofVal, args: KofVal[], line: number): Promise<KofVal> {
    ...
    if (f.enumStatic) {
      ...
      const ed = this.enums.get(en)!;
      if (f.method === "values") return ed.consts.map((c) => kofObj("enumConst", { enumType: en, label: c }));
    ...
    if (f.listOp) return await this.listOp(String(f.listOp), f.recv as KofList, args, line, f.scope as Scope, (f.self as KofObj) ?? null);
    if (f.mapOp) return await this.mapOp(String(f.mapOp), f.recv as KofMap, args, line, f.scope as Scope);
    if (f.setOp) return await this.setOp(String(f.setOp), f.recv as KofSet, args, f.scope as Scope, line);
    if (f.strOp) return this.stringMethod(String(f.recv), String(f.strOp), args, line);
    if (f.recAccessor) return (f.recAccessor.components as { name: string; value: KofVal }[]).find((c) => c.name === f.field)?.value ?? null;
    if (f.recSpecial) {
  ...
  private async construct(name: string, args: KofVal[], line: number): Promise<KofVal> {
    ...
    if (rec) {
      if (args.length !== rec.params.length)
        throw new KofDiag(`record \`${name}\` espera ${rec.params.length} componente(s), recebeu ${args.length}`, "SEM013", line);
      const comp = rec.params.map((p, i) => ({ name: p.name, value: coerce(p.type, args[i] ?? null) }));
  ...
  private async callMethodOn(d: Fn, args: KofVal[], obj: KofObj, _clsName: string): Promise<KofVal> {
    const scope = new Scope(null);
    for (let i = 0; i < d.params.length; i++) {
      const p = d.params[i]!;
  ...
  private async callMethod(recv: KofVal, name: string, args: KofVal[], scope: Scope, self: KofObj | null, line: number): Promise<KofVal> {
    ...
    if (typeof recv === "number" || typeof recv === "bigint" || typeof recv === "boolean" || isKofChar(recv) || isFp(recv as FpVal)) {
      if (name === "toString") {
        if (typeof recv === "number" && Number.isInteger(recv) && args.length) return (recv >>> 0).toString(Number(asInt(args[0])) >>> 0 ? Number(asInt(args[0])) : 10);
  ...
  private stringMethod(s: string, name: string, args: KofVal[], line: number): KofVal {
    switch (name) {
      ...
      case "startsWith":
        if (args.length >= 2) return strStartsWith2(s, kofStr(args[0] ?? ""), Number(asInt(args[1])));
      ...
      case "lastIndexOf":
        if (args.length >= 2) return strLastIndexOf2(s, kofStr(args[0] ?? ""), Number(asInt(args[1])));
      ...
      case "trim":
        return s.trim();
      case "replace": {
        const f = isKofChar(args[0]) ? String.fromCharCode((args[0] as KofChar).__kofChar) : kofStr(args[0] ?? "");
        const t = isKofChar(args[1]) ? String.fromCharCode((args[1] as KofChar).__kofChar) : kofStr(args[1] ?? "");
  ...
  private async listOp(op: string, list: KofList, args: KofVal[], line: number, scope: Scope, self: KofObj | null): Promise<KofVal> {
    ...
    switch (op) {
      case "get": {
        ...
        if (i < 0 || i >= list.length)
          throw new KofRuntimeError(
            isArr ? `Array index out of bounds: ${i} (length ${list.length})` : `Index out of bounds: ${i} (size ${list.length})`,
          );
        return list[i]!;
      }
      case "set": {
        const i = Number(asInt(args[0]));
        if (i < 0 || i >= list.length) throw new KofRuntimeError(`Index out of bounds: ${i} (size ${list.length})`);
        list[i] = args[1]!;
        return null;
      }
      case "size":
      ...
      case "push":
      case "append":
        if (isArr) throw new KofDiag("array é fixo — sem .add(): use List via listOf()", "SEM028", line);
      ...
      case "filter": {
        const outList: KofVal[] = [];
        for (const v of list) if (truthy(await this.invoke(args[0] ?? null, [v], line))) outList.push(v);
        return outList;
      }
      case "reduce": {
        let acc = args[0] ?? null;
      ...
      case "toString":
        return kofStr(list);
      default:
        if (isArr && ["add", "get", "set", "remove", "clear", "size", "map", "filter", "reduce"].includes(op))
          throw new KofDiag(`array é fixo — sem \`.${op}()\`: use arr[i] e arr.length`, "SEM028", line);
  ...
  private async mapOp(op: string, m: KofMap, args: KofVal[], line: number, _scope: Scope): Promise<KofVal> {
    switch (op) {
      case "put":
  ...
  private async setOp(op: string, s: KofSet, args: KofVal[], _scope: Scope, line = 0): Promise<KofVal> {
    switch (op) {
      case "add": {
  ...
  private evalSpawn(body: Expr, scope: Scope, self: KofObj | null): KofVal {
    ...
    const handle = kofObj("handle", {}) as KofObj & { done: boolean; value?: KofVal; error?: unknown };
    handle.done = false;
    this.activeTasks++;
    const p = (async () => {
      try {
  ...
  private async evalAwait(h: KofVal): Promise<KofVal> {
    ...
    if (isObj(h) && h.kind === "handle") {
      ...
      while (!handle.done) {
        await sleep(1);
        if (Date.now() > this.deadline) throw new KofRuntimeError("await timeout (loop sem fim?)", undefined, true);
      }
  ...
  async binary(op: string, a: KofVal, b: KofVal, line: number): Promise<KofVal> {
    ...
    if (op === "+" && (typeof a === "string" || typeof b === "string" || isKofChar(a) || isKofChar(b)))
      return kofStr(a) + kofStr(b);
    if (["<", "<=", ">", ">="].includes(op) && (typeof a === "string" || typeof b === "string"))
      throw new KofDiag("ordem lexicográfica de String é `compareTo`, não `<` — use `a.compareTo(b) < 0`", "SEM053", line);
    if (isObj(a) || isObj(b)) {
      if (op === "+" && (isObj(a) || isObj(b)) && (typeof a === "string" || typeof b === "string")) return kofStr(a) + kofStr(b);
    ...
    const aNum = asNum(a);
    const bNum = asNum(b);
    const bothInt = !aFp && !bFp && Number.isInteger(aNum) && Number.isInteger(bNum) && !isKofChar(a) && !isKofChar(b);
    if (bothInt && op !== "/") {
    ...
    if (bothInt && op === "/") {
      const x = wrapInt(aNum as number);
      const y = wrapInt(bNum as number);
      if (typeof a === "number" && typeof b === "number" && !isFp(a as Val) && !isFp(b as Val) && Number.isInteger(a) && Number.isInteger(b))
        return intDiv(x, y);
    }
    const x = Number(aNum);
    const y = Number(bNum);
    const mk = (n: number): KofVal => (aFp || bFp ? { __kofFloat: aFp && (a as FpVal).isFloat ? Math.fround(n) : n, isFloat: aFp ? (a as FpVal).isFloat && (!bFp || (b as FpVal).isFloat) : (b as FpVal).isFloat } : n);
    switch (op) {
      case "+":
        return mk(x + y);
      case "-":
  ...
  private async evalCollection(name: string, argsE: Expr[], scope: Scope, self: KofObj | null, line: number): Promise<KofVal> {
    const args: KofVal[] = [];
...
function catchMatches(type: string, thrown: KofVal): boolean {
  ...
  switch (t) {
    ...
    case "Double":
      return isFp(thrown as Val | FpVal) || (typeof thrown === "number" && !Number.isInteger(thrown));
    case "Bool":
...
function coerce(type: string | null, v: KofVal): KofVal {
  ...
  if (t === "Float" && typeof v === "number") return { __kofFloat: Math.fround(v), isFloat: true } as FpVal;
...
function defaultOf(type: string | null): KofVal {
  if (!type) return null;
  const t = type.replace(/\?$/, "").replace(/<.*$/, "");
  if (t === "Int" || t === "Char" || t === "Short" || t === "Byte") return t === "Char" ? ({ __kofChar: 0 } as KofChar) : 0;
  if (t === "Long") return 0n;
...
function parseLongStrict(s: string): bigint {
  ...
  const b = BigInt(t);
  if (b > 9223372036854775807n || b < -9223372036854775808n) throw new KofRuntimeError(`Cannot parse "${t}" as Long`);
  return b;
}
...
function parseStatic(ns: string, fn: string, s: string): KofVal {
  ...
  if (fn === "parseFloat") return { __kofFloat: Math.fround(parseDoubleStrict(s)), isFloat: true } as FpVal;
...
function jsonStringify(v: KofVal): string {
  ...
  if (isObj(v)) {
    if (v.kind === "record") {
      ...
      return "{" + comp.map((c) => jsonQuote(c.name) + ":" + jsonStringify(c.value)).join(",") + "}";
    }
...
function fromJsValue(x: unknown): KofVal {
  ...
  if (typeof x === "number") return Number.isInteger(x) ? x : { __kofFloat: x, isFloat: false } as FpVal;
